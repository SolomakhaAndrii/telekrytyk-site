...
<p className={styles.postDate}>{new Date(new Date(post.created_at).getTime() + 3 * 60 * 60 * 1000).toLocaleString("uk-UA", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}</p>                  {post.link && <a href={post.link} target="_blank" rel="noopener noreferrer" className={styles.postLink}>{post.link_label || post.link}</a>}
                </div>
